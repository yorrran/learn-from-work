...
export class DebounceObservableComponent implements OnInit, OnDestroy {
  name1: string ='';
  name2: string = '';
  ...
  ngOnInit() {
    const input1 = fromEvent<any>(this.input1.nativeElement, "keyup")
      .pipe(debounceTime(200));

    const input2 = fromEvent<any>(this.input2.nativeElement, "keyup")
      .pipe(debounceTime(200));

    this.inputSubscription = input1.pipe(switchMap(event1=>{
